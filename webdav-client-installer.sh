...
OCS_JAR_URL="https://github.com/Zimbra-Community/seafile/raw/master/extension/out/artifacts/OCS_jar/OCS.jar"
...
if [ -f ${OWNCLOUD_EXTENSION_PATH}/config.properties ]; then
   cat ${OWNCLOUD_EXTENSION_PATH}/config.properties | grep -v seafile | grep -v owncloud_zimlet_extra_toolbar_button_title > $TMPFOLDER/upgrade/config.properties
...
sed -i "s^Nextcloud^Seafile^g" $TMPFOLDER/owncloud-zimlet/zimlet/tk_barrydegraaff_owncloud_zimlet.xml
wget https://github.com/Zimbra-Community/seafile/raw/master/seafile.png -O $TMPFOLDER/owncloud-zimlet/zimlet/icon.png
...
owncloud_zimlet_extra_toolbar_button_title=
